...
// Writes row to 8 -> 1 line mux chip as BCD
static bool select_col(uint8_t line) {
  ...
  //TODO: in case keyboard has unused cols
  return true;
}
...
void matrix_read_cols_on_row(matrix_row_t current_matrix[], uint8_t current_row) {
    ...
    for (uint8_t col_index = 0; col_index < MATRIX_COLS; col_index++, row_shifter <<= 1) {
        ...
        uint8_t pin_state = readMatrixPin(col_pin);
...
void matrix_init(void) {
    ...
    // Set col mux to col 0 (can't turn it off, Strobe is pulled to ground in hw)
    select_col(0);
