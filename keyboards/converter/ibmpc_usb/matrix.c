...
#if defined(IBMPC_CLOCK_BIT1) && defined(IBMPC_DATA_BIT1)
IBMPCConverter_t converter1;
#endif

void keyboard_pre_init_kb(void)
{
    ibmpcconverter_init(&converter0, &interface0);
#if defined(IBMPC_CLOCK_BIT1) && defined(IBMPC_DATA_BIT1)
    ibmpcconverter_init(&converter1, &interface1);
#endif
}

void matrix_clear(void)
{
    ibmpcconverter_matrix_clear();
}
...
void matrix_init(void)
{
    ...
    ibmpc_host_enable(&interface0);
#if defined(IBMPC_CLOCK_BIT1) && defined(IBMPC_DATA_BIT1)
    ibmpc_host_enable(&interface1);
#endif
}

matrix_row_t matrix_get_row(uint8_t row)
{
    return ibmpcconverter_matrix_get_row(row);
}

uint8_t matrix_scan(void)
{
    ibmpcconverter_process_interface(&converter0);
#if defined(IBMPC_CLOCK_BIT1) && defined(IBMPC_DATA_BIT1)
    ibmpcconverter_process_interface(&converter1);
#endif
    return 1;
}
...
{
    ibmpcconverter_set_led(&converter0, usb_led);
    #if defined(IBMPC_CLOCK_BIT1) && defined(IBMPC_DATA_BIT1)
    ibmpcconverter_set_led(&converter1, usb_led);
    #endif
}
...
void ibmpcconverter_set_led(IBMPCConverter_t* self, uint8_t usb_led)
{
    // Sending before keyboard recognition may be harmful for XT keyboard
    if (self->keyboard_kind == NONE) return;
    ...
    if (self->keyboard_kind == PC_XT) return;
    if (self->keyboard_kind == PC_MOUSE) return;
    ...
    ibmpc_host_set_led(self->ibmpc, ibmpc_led);
}

int16_t ibmpcconverter_read_wait(IBMPCConverter_t* self, uint16_t wait_ms)
{
    uint16_t start = timer_read();
    int16_t code;
    while ((code = ibmpc_host_recv(self->ibmpc)) == -1 && timer_elapsed(start) < wait_ms);
 ...
 */
uint16_t ibmpc_converter_read_keyboard_id(IBMPCConverter_t* self)
{
    uint16_t id = 0;
    int16_t  code = 0;
    ...
    if (self->ibmpc->protocol == IBMPC_PROTOCOL_AT_Z150) return 0xFFFD;

    // XT doesn't response
    if (self->ibmpc->protocol == IBMPC_PROTOCOL_XT_IBM) return 0xFFFC;
    if (self->ibmpc->protocol == IBMPC_PROTOCOL_XT_CLONE) return 0xFFFB;

    // Disable
    //code = ibmpc_host_send(0xF5);

    // Read ID
    code = ibmpc_host_send(self->ibmpc, 0xF2);
    ...
    code = ibmpcconverter_read_wait(self, 500);
...
DONE:
    // Enable
    //code = ibmpc_host_send(self->ibmpc, 0xF4);
...
uint8_t ibmpcconverter_process_interface(IBMPCConverter_t* self)
{
    if (self->ibmpc->error) {
        xprintf("\n%u ERR:%02X ISR:%04X ", timer_read(), self->ibmpc->error, self->ibmpc->isr_debug);
        ...
        if (!(self->ibmpc->error & (IBMPC_ERR_SEND | IBMPC_ERR_FULL))) {
            self->state = ERROR;
            if (self->ibmpc->error == IBMPC_ERR_PARITY_AA) {
                // AT/XT Auto-Switching support
                // https://github.com/tmk/tmk_keyboard/wiki/IBM-PC-Keyboard-Converter#atxt-auto-switching
                self->state = ERROR_PARITY_AA;
            }
        }

        // clear or process error
        self->ibmpc->error = IBMPC_ERR_NONE;
        self->ibmpc->isr_debug = 0;
    }

    // check protocol change AT/XT
    if (self->ibmpc->protocol && self->ibmpc->protocol != self->current_protocol) {
        xprintf("\n%u PRT:%02X ISR:%04X ", timer_read(), self->ibmpc->protocol, self->ibmpc->isr_debug);
        ...
        if (((self->current_protocol&IBMPC_PROTOCOL_XT) && (self->ibmpc->protocol&IBMPC_PROTOCOL_AT)) ||
            ((self->current_protocol&IBMPC_PROTOCOL_AT) && (self->ibmpc->protocol&IBMPC_PROTOCOL_XT))) {
            if (self->state == LOOP) {
                xprintf("[CHG] ");
                self->state = ERROR;
            }
        ...
        self->current_protocol = self->ibmpc->protocol;
        self->ibmpc->isr_debug = 0;
    }
    ...
    switch (self->state) {
        case INIT:
            ...
            self->init_time = timer_read();
            ibmpc_host_isr_clear(self->ibmpc);
            ibmpc_host_enable(self->ibmpc);
            self->state = WAIT_SETTLE;
            break;
        case WAIT_SETTLE:
            while (ibmpc_host_recv(self->ibmpc) != -1) ; // read data

            // wait for keyboard to settle after plugin
            if (timer_elapsed(self->init_time) > 3000) {
                self->state = AT_RESET;
            }
            break;
        case AT_RESET:
            ...
            // reset command
            if (0xFA == ibmpc_host_send(self->ibmpc, 0xFF)) {
                self->state = WAIT_AA;
            } else {
                self->state = XT_RESET;
            }
        ...
        case XT_RESET:
            ...
            ibmpc_host_disable(self->ibmpc);   // soft reset: Clock Lo/Data Hi
            IBMPC_RST_LO();         // hard reset: Reset pin Lo

            self->init_time = timer_read();
            self->state = XT_RESET_WAIT;
            break;
        case XT_RESET_WAIT:
            if (timer_elapsed(self->init_time) > 500) {
                self->state = XT_RESET_DONE;
            }
            break;
        case XT_RESET_DONE:
            IBMPC_RST_HIZ();        // hard reset: Reset pin HiZ
            ibmpc_host_isr_clear(self->ibmpc);
            ibmpc_host_enable(self->ibmpc);    // soft reset: idle(Clock Hi/Data Hi)
        ...
        case WAIT_AA:
            ...
            if (timer_elapsed(self->init_time) > 10000) {
                self->state = READ_ID;
            }
            */
            if (ibmpc_host_recv(self->ibmpc) != -1) {  // wait for AA
                xprintf("W%u ", timer_read());
                self->init_time = timer_read();
                self->state = WAIT_AABF;
            }
            break;
        case WAIT_AABF:
            ...
            if (ibmpc_host_recv(self->ibmpc) != -1) {  // wait for BF
                xprintf("W%u ", timer_read());
                self->init_time = timer_read();
                self->state = WAIT_AABFBF;
            }
            break;
        case WAIT_AABFBF:
            if (timer_elapsed(self->init_time) > 500) {
            ...
            break;
        case READ_ID:
            self->keyboard_id = ibmpc_converter_read_keyboard_id(self);
            xprintf("R%u ", timer_read());

            if (0x0000 == self->keyboard_id) {            // CodeSet2 AT(IBM PC AT 84-key)
                self->keyboard_kind = PC_AT;
            } else if (0xFFFF == self->keyboard_id) {     // No keyboard or XT
                self->keyboard_kind = PC_XT;
            } else if (0xFFFE == self->keyboard_id) {     // CodeSet2 PS/2 fails to response?
                self->keyboard_kind = PC_AT;
            } else if (0xFFFD == self->keyboard_id) {     // Zenith Z-150 AT
                self->keyboard_kind = PC_AT;
            } else if (0xFFFC == self->keyboard_id) {     // IBM XT
                self->keyboard_kind = PC_XT;
            } else if (0xFFFB == self->keyboard_id) {     // Clone XT
                self->keyboard_kind = PC_XT;
            } else if (0x00FF == self->keyboard_id) {     // Mouse is not supported
                self->keyboard_kind = PC_MOUSE;
            } else if (0xAB85 == self->keyboard_id || // IBM 122-key Model M, NCD N-97
                       0xAB86 == self->keyboard_id || // Cherry G80-2551, IBM 1397000
                       0xAB92 == self->keyboard_id) { // IBM 5576-001
                ...
                if ((0xFA == ibmpc_host_send(self->ibmpc, 0xF0)) &&
                    (0xFA == ibmpc_host_send(self->ibmpc, 0x03))) {
                    // switch to code set 3
                    self->keyboard_kind = PC_TERMINAL;
                } else {
                    self->keyboard_kind = PC_AT;
                }
            } else if (0xAB90 == self->keyboard_id || // IBM 5576-002
                       0xAB91 == self->keyboard_id) { // IBM 5576-003 or Televideo DEC
            ...
            } else if (0xBFB0 == self->keyboard_id) {     // IBM RT Keyboard
                // https://github.com/tmk/tmk_keyboard/wiki/IBM-PC-AT-Keyboard-Protocol#bfb0
                // TODO: LED indicator fix
                //self->keyboard_kind = PC_TERMINAL_IBM_RT;
                self->keyboard_kind = PC_TERMINAL;
            } else if (0xAB00 == (self->keyboard_id & 0xFF00)) {  // CodeSet2 PS/2
                self->keyboard_kind = PC_AT;
            } else if (0xBF00 == (self->keyboard_id & 0xFF00)) {  // CodeSet3 Terminal
                self->keyboard_kind = PC_TERMINAL;
            } else if (0x7F00 == (self->keyboard_id & 0xFF00)) {  // CodeSet3 Terminal 1394204
                self->keyboard_kind = PC_TERMINAL;
            } else {
                xprintf("\nUnknown ID: Report to TMK ");
                if ((0xFA == ibmpc_host_send(self->ibmpc, 0xF0)) &&
                    (0xFA == ibmpc_host_send(self->ibmpc, 0x02))) {
                    // switch to code set 2
                    self->keyboard_kind = PC_AT;
                } else if ((0xFA == ibmpc_host_send(self->ibmpc, 0xF0)) &&
                           (0xFA == ibmpc_host_send(self->ibmpc, 0x03))) {
                    // switch to code set 3
                    self->keyboard_kind = PC_TERMINAL;
                } else {
            ...
            xprintf("\nID:%04X(%s%s) ", self->keyboard_id, KEYBOARD_KIND_STR(self->keyboard_kind), ID_STR(self->keyboard_id));
        ...
        case SETUP:
            ...
            switch (self->keyboard_kind) {
                ...
                case PC_TERMINAL:
                    ...
                    ibmpc_host_send(self->ibmpc, 0xF8);
                ...
                case PC_MOUSE: {
                    uint8_t s[3];
                    ibmpc_host_send(self->ibmpc, 0xF5);  // Disable
                    ibmpc_host_send(self->ibmpc, 0xEA);  // Set Stream Mode
                    mouse_read_status(self, s);

                    // Logitech Magic Status
                    // https://github.com/torvalds/linux/blob/master/drivers/input/mouse/logips2pp.c#L352
                    xprintf("\nLMS: ");
                    ibmpc_host_send(self->ibmpc, 0xE8); ibmpc_host_send(self->ibmpc, 0x00);
                    ibmpc_host_send(self->ibmpc, 0xE6); ibmpc_host_send(self->ibmpc, 0xE6); ibmpc_host_send(self->ibmpc, 0xE6);
                    mouse_read_status(self, s);
                    if (s[0] == 0 || s[1] == 0) {
                        // Not Logitech
                        goto MOUSE_INTELLI;
                    ...
                    ibmpc_host_send(self->ibmpc, 0xE6);
                    ibmpc_host_send(self->ibmpc, 0xE8); ibmpc_host_send(self->ibmpc, 0x00);
                    ibmpc_host_send(self->ibmpc, 0xE8); ibmpc_host_send(self->ibmpc, 0x03);
                    ibmpc_host_send(self->ibmpc, 0xE8); ibmpc_host_send(self->ibmpc, 0x02);
                    ibmpc_host_send(self->ibmpc, 0xE8); ibmpc_host_send(self->ibmpc, 0x01);
...
MOUSE_INTELLI:
                    ...
                    xprintf("\nINT: ");
                    ibmpc_host_send(self->ibmpc, 0xF3); ibmpc_host_send(self->ibmpc, 0xC8);
                    ibmpc_host_send(self->ibmpc, 0xF3); ibmpc_host_send(self->ibmpc, 0x64);
                    ibmpc_host_send(self->ibmpc, 0xF3); ibmpc_host_send(self->ibmpc, 0x50);
                    mouse_id = ((ibmpc_converter_read_keyboard_id(self) >> 8) == MOUSE_INTELLI ? MOUSE_INTELLI : MOUSE_DEFAULT);
                    ...
                    ibmpc_host_send(self->ibmpc, 0xF3); ibmpc_host_send(self->ibmpc, 0xC8);
                    ibmpc_host_send(self->ibmpc, 0xF3); ibmpc_host_send(self->ibmpc, 0x50);
                    mouse_id = ((ibmpc_converter_read_keyboard_id(self) >> 8) == MOUSE_EXPLORER ? MOUSE_EXPLORER : mouse_id);

                    // Not Intellimouse
                    if (mouse_id == 0) {
                        xprintf("\nDEF: ");
                        ibmpc_host_send(self->ibmpc, 0xF6);  // Set Default
                    }

MOUSE_DONE:
                    //ibmpc_host_send(self->ibmpc, 0xEA);  // Set Stream Mode
                    ibmpc_host_send(self->ibmpc, 0xF4);  // Enable
                    mouse_read_status(self, s);
        ...
        case LOOP:
            {
                uint16_t code = ibmpc_host_recv(self->ibmpc);
                if (code == -1) {
                    // no code
                    break;
                }
                ...
                switch (self->keyboard_kind) {
                    case PC_XT:
                        if (ibmpcconverter_process_cs1(self, code) == -1) self->state = ERROR;
                        break;
                    case PC_AT:
                        if (ibmpcconverter_process_cs2(self, code) == -1) self->state = ERROR;
                        break;
                    case PC_TERMINAL:
                        if (ibmpcconverter_process_cs3(self, code) == -1) self->state = ERROR;
                        break;
#ifdef IBMPC_MOUSE_ENABLE
                    case PC_MOUSE: {
                        // Logitec Mouse Data:
                        ...
                        b0 = code;
                        b1 = ibmpc_host_recv_response(self->ibmpc);
                        if (b1 == -1) { self->state = ERROR; break; }
                        b2 = ibmpc_host_recv_response(self->ibmpc);
                        if (b2 == -1) { self->state = ERROR; break; }
                        ...
                        switch (mouse_id) {
                            case MOUSE_INTELLI:
                                b3 = ibmpc_host_recv_response(self->ibmpc);
                                if (b3 == -1) break;
                                v = b3 & 0xFF;
                                break;
                            case MOUSE_EXPLORER:
        ...
        case ERROR_PARITY_AA:
            {
                ...
                uint16_t code = ibmpc_host_send(self->ibmpc, 0xFE);
                if (0xAA == code) {
                    self->state = READ_ID;
                    break;
                }
        ...
        case ERROR:
            ...
            // reinit state
            ibmpcconverter_init(self, self->ibmpc);
            clear_stuck_keys();
            self->state = INIT;
            break;
        default:
...
int8_t ibmpcconverter_process_cs1(IBMPCConverter_t* self, uint8_t code)
{
    switch (self->state_cs1) {
        case CS1_INIT:
            switch (code) {
                case 0xFF:  // Error/Overrun([3]p.26)
                    ...
                    break;
                case 0xE0:
                    self->state_cs1 = CS1_E0;
                    break;
                case 0xE1:
                    self->state_cs1 = CS1_E1;
                    break;
                default:
                    if (code < 0x80)
                        matrix_make(self, code);
                    else
                        matrix_break(self, code & 0x7F);
                    break;
            }
            break;
        case CS1_E0:
            switch (code) {
                ...
                default:
                    if (code < 0x80)
                        matrix_make(self, cs1_e0code(code));
                    else
                        matrix_break(self, cs1_e0code(code & 0x7F));
                    self->state_cs1 = CS1_INIT;
                    break;
            }
            break;
        case CS1_E1:
            switch (code) {
                case 0x1D:
                    self->state_cs1 = CS1_E1_1D;
                    break;
                case 0x9D:
                    self->state_cs1 = CS1_E1_9D;
                    break;
                default:
                    self->state_cs1 = CS1_INIT;
                    break;
 ...
 *
 */
uint8_t ibmpcconverter_cs2_e0code(IBMPCConverter_t* self, uint8_t code) {
    switch(code) {
        case 0x11:  if (0xAB90 == self->keyboard_id || 0xAB91 == self->keyboard_id)
                        return 0x13; // Hiragana(5576) -> KANA
                    else
                        return 0x0F; // right alt

        case 0x41:  if (0xAB90 == self->keyboard_id || 0xAB91 == self->keyboard_id)
...
int8_t ibmpcconverter_process_cs2(IBMPCConverter_t* self, uint8_t code)
{
    switch (self->state_cs2) {
        case CS2_INIT:
            if (0xAB90 == self->keyboard_id || 0xAB91 == self->keyboard_id) {
            ...
            switch (code) {
                case 0x00:  // Error/Overrun([3]p.26)
                    ...
                    break;
                case 0xE0:
                    self->state_cs2 = CS2_E0;
                    break;
                case 0xF0:
                    self->state_cs2 = CS2_F0;
                    break;
                case 0xE1:
                    self->state_cs2 = CS2_E1;
                    break;
#ifdef CS2_80CODE_SUPPORT
                case 0x80:
                    self->state_cs2 = CS2_80;
                    break;
#endif
                case 0x83:  // F7
                    matrix_make(self, 0x02);
                    self->state_cs2 = CS2_INIT;
                    break;
                case 0x84:  // Alt'd PrintScreen
                    matrix_make(self, 0x7F);
                    self->state_cs2 = CS2_INIT;
                    break;
        ...
        case CS2_E0:    // E0-prefixed
            switch (code) {
                ...
                case 0x59:  // to be ignored
                    ...
                    break;
                case 0xF0:
                    self->state_cs2 = CS2_E0_F0;
                    break;
                default:
                    self->state_cs2 = CS2_INIT;
                    if (code < 0x80) {
                        matrix_make(self, ibmpcconverter_cs2_e0code(self, code));
                    } else {
                        matrix_clear();
                        xprintf("!CS2_E0!\n");
                        return -1;
        ...
        case CS2_F0:    // Break code
            ...
            switch (code) {
                case 0x83:  // F7
                    matrix_break(self, 0x02);
                    self->state_cs2 = CS2_INIT;
                    break;
                case 0x84:  // Alt'd PrintScreen
                    matrix_break(self, 0x7F);
                    self->state_cs2 = CS2_INIT;
                    break;
                default:
                    self->state_cs2 = CS2_INIT;
                    if (code < 0x80) {
                        matrix_break(self, code);
                    } else {
                        matrix_clear();
                        xprintf("!CS2_F0! %02X\n", code);
                        return -1;
        ...
        case CS2_E0_F0: // Break code of E0-prefixed
            switch (code) {
                ...
                default:
                    self->state_cs2 = CS2_INIT;
                    if (code < 0x80) {
                        matrix_break(self, ibmpcconverter_cs2_e0code(self, code));
                    } else {
                        matrix_clear();
                        xprintf("!CS2_E0_F0!\n");
                        return -1;
        ...
        case CS2_E1:
            switch (code) {
                case 0x14:
                    self->state_cs2 = CS2_E1_14;
                    break;
                case 0xF0:
                    self->state_cs2 = CS2_E1_F0;
                    break;
                default:
                    self->state_cs2 = CS2_INIT;
            }
        ...
        case CS2_E1_F0_14:
            switch (code) {
                case 0xF0:
                    self->state_cs2 = CS2_E1_F0_14_F0;
                    break;
                default:
                    self->state_cs2 = CS2_INIT;
            }
        ...
        case CS2_80:
            switch (code) {
                case 0xF0:
                    self->state_cs2 = CS2_80_F0;
                    break;
                default:
                    self->state_cs2 = CS2_INIT;
                    matrix_make(self, cs2_80code(code));
            }
            break;
        case CS2_80_F0:
            switch (code) {
                default:
                    self->state_cs2 = CS2_INIT;
                    matrix_break(self, cs2_80code(code));
            }
            break;
#endif
        default:
...
int8_t ibmpcconverter_process_cs3(IBMPCConverter_t* self, uint8_t code)
{
    switch (code) {
        case 0xAA:  // BAT code
        case 0xFC:  // BAT code
    ...
    switch (self->state_cs3) {
        case CS3_READY:
            if (0xAB92 == self->keyboard_id) {
            ...
            switch (code) {
                ...
                case 0x84:  // Keypad *
                    ...
                    break;
                case 0x85:  // Muhenkan
                    matrix_make(self, 0x68);
                    break;
                case 0x86:  // Henkan
                    matrix_make(self, 0x78);
                    break;
                case 0x87:  // Hiragana
                    matrix_make(self, 0x00);
                    break;
                case 0x8B:  // Left GUI
                    matrix_make(self, 0x01);
                    break;
                case 0x8C:  // Right GUI
                    matrix_make(self, 0x09);
                    break;
                case 0x8D:  // Application
                    matrix_make(self, 0x0A);
                    break;
#ifdef G80_2551_SUPPORT
                case 0x80:  // G80-2551 four extra keys around cursor keys
                    self->state_cs3 = CS3_G80;
                    break;
#endif
                default:    // normal key make
                    if (code < 0x80) {
        ...
        case CS3_F0:    // Break code
            ...
            switch (code) {
                ...
                case 0x84:  // Keypad *
                    ...
                    break;
                case 0x85:  // Muhenkan
                    matrix_break(self, 0x68);
                    break;
                case 0x86:  // Henkan
                    matrix_break(self, 0x78);
                    break;
                case 0x87:  // Hiragana
                    matrix_break(self, 0x00);
                    break;
                case 0x8B:  // Left GUI
                    matrix_break(self, 0x01);
                    break;
                case 0x8C:  // Right GUI
                    matrix_break(self, 0x09);
                    break;
                case 0x8D:  // Application
                    matrix_break(self, 0x0A);
                    break;
                default:
                    if (code < 0x80) {
                        matrix_break(self, code);
        ...
        case CS3_G80:   // G80-2551 four extra keys around cursor keys
            switch (code) {
                case (0x26):    // TD= -> JYEN
                    matrix_make(self, 0x5D);
                    break;
                case (0x25):    // page with edge -> NUHS
                    matrix_make(self, 0x53);
                    break;
                case (0x16):    // two pages -> RO
                    matrix_make(self, 0x51);
                    break;
                case (0x1E):    // calc -> KANA
                    matrix_make(self, 0x00);
                    break;
                case (0xF0):
                    self->state_cs3 = CS3_G80_F0;
                    return 0;
                default:
                    // Not supported
                    matrix_clear();
        ...
        case CS3_G80_F0:
            switch (code) {
                case (0x26):    // TD= -> JYEN
                    matrix_break(self, 0x5D);
                    break;
                case (0x25):    // page with edge -> NUHS
                    matrix_break(self, 0x53);
                    break;
                case (0x16):    // two pages -> RO
                    matrix_break(self, 0x51);
                    break;
                case (0x1E):    // calc -> KANA
                    matrix_break(self, 0x00);
                    break;
